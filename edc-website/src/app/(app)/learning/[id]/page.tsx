...
export default function CoursePlayerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!data || !data.course) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center gap-4">
        ...
        <Button onClick={() => window.location.href = "/learning"} variant="outline">Return to Learning Hub</Button>
      </div>
    )
  }
  ...
  const renderVideoPlayer = (url: string) => {
    ...
    if (url.includes('youtube.com') || url.includes('youtu.be')) {
      ...
      if (videoId) {
        return <iframe src={`https://www.youtube.com/embed/${videoId}`} className="w-full h-full rounded-md border bg-black" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
      }
    }
